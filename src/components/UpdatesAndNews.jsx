// src/components/UpdatesAndNews.jsx - Strictly Separated Official Government Updates & Daily News

import React, { useState, useEffect } from 'react';
import { Landmark, Newspaper, AlertCircle, Bell, ExternalLink, Calendar, ShieldCheck, ChevronRight } from 'lucide-react';

export default function UpdatesAndNews({ govtUpdates: initialGovt, dailyNews: initialNews, initialTab }) {
  const [activeTab, setActiveTab] = useState(initialTab === 'news' ? 'news' : 'govt');
  const [govtUpdates, setGovtUpdates] = useState(initialGovt || []);
  const [dailyNews, setDailyNews] = useState(initialNews || []);

  // Sync with parent props if they change (e.g. after API fetch updates state)
  useEffect(() => { if (initialGovt?.length > 0) setGovtUpdates(initialGovt); }, [initialGovt]);
  useEffect(() => { if (initialNews?.length > 0) setDailyNews(initialNews); }, [initialNews]);
    ...
    <div style={{ maxWidth: '1200px', margin: '0 auto', padding: '24px 16px' }}>
      ...
      <div style={{
        ...
        <div style={{ display: 'flex', gap: '16px' }}>
          ...
          >
            <Landmark size={20} /> Official Government Updates
          </button>
          ...
          >
            <Newspaper size={20} /> Daily News & Media
          </button>
        </div>
        ...
        <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fill, minmax(340px, 1fr))', gap: '20px' }}>
          {dailyNews.map(item => (
            ...
            >
              ...
              <a
                href="#news"
                onClick={(e) => { e.preventDefault(); alert(`Opening article from ${item.source}...`); }}
                style={{
                  fontSize: '0.8rem',
                  fontWeight: 700,
                  color: '#0B5ED7',
